...
def find_hr_image(lr_filename, hr_dir):
    """
    LR dosya adına göre HR dosyayı güvenli şekilde bulur.
    Desteklenen yapılar:
    - resim_lr.png -> resim.png
    - resim.png -> resim.png
    """
...
def evaluate_model():
    ...
    if not os.path.exists(lr_dir) or not os.path.exists(hr_dir):
        print("❌ HATA: lr_images veya hr_images klasörü yok.")
        return
    ...
    print(f"📂 Toplam {len(files)} resim kontrol edilecek...")
    print("-" * 75)
    ...
    for idx, filename in enumerate(files):
        ...
        if hr_path is None:
            print(f"⚠️ HR bulunamadı, atlandı: {filename}")
            continue
        ...
        sr_lp = (sr_lp * 2 - 1).to(DEVICE)
        hr_lp = (hr_lp * 2 - 1).to(DEVICE)
    ...
    if tested == 0:
        print("❌ Hiç eşleşen HR-LR çifti bulunamadı.")
        return
    ...
    print("-" * 42)
    if gain > 0:
        print(f"🚀 BAŞARILI! Ortalama {gain:.4f} dB PSNR kazancı.")
    else:
        print("⚠️ Bicubic PSNR olarak daha iyi.")
